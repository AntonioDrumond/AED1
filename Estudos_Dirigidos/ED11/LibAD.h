...
{
    int buffer = 0, leng = len-1, smallerPos;
    for (int i=0; i<leng; i++)
    {
        smallerPos = i;
        for(int j=i+1; j<len; j++)
        {
            if(arr[smallerPos] > arr[j]){
                smallerPos = j;
            }
        }
        buffer = arr[i];
        arr[i] = arr[smallerPos];
        arr[smallerPos] = buffer;
    }
}

void sortArrayReverse (int arr[], int len)
{
    int buffer = 0, leng = len-1, biggerPos;
    for (int i=0; i<leng; i++)
    {
        biggerPos = i;
        for(int j=i+1; j<len; j++)
